...
private:
	...
	std::vector<UINT> firstIndices;
	std::vector<UINT> firstVertices;
...
void BoxApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		for (UINT i = 0; i < firstVertices.size(); ++i)
		{
			md3dImmediateContext->DrawIndexed(numIndices[i], firstIndices[i], firstVertices[i]);		
		}
    }
...
void BoxApp::BuildGeometryBuffers()
{
	...
	firstVertices.push_back(0);

	std::vector<Vertex> vertices2 =
	{
		{ XMFLOAT3(+0.0f, +3.0f, +0.0f), XMFLOAT4((const float*)&Colors::LightSteelBlue) },
		{ XMFLOAT3(+1.0f, +2.0f, -1.0f), XMFLOAT4((const float*)&Colors::Silver) },
		{ XMFLOAT3(-1.0f, +2.0f, -1.0f), XMFLOAT4((const float*)&Colors::White) },
		{ XMFLOAT3(+1.0f, +2.0f, +1.0f), XMFLOAT4((const float*)&Colors::Yellow) },
		{ XMFLOAT3(-1.0f, +2.0f, +1.0f), XMFLOAT4((const float*)&Colors::Cyan) }
	};
	firstVertices.push_back(0 + vertices1.size());
	...
	firstIndices.push_back(0);
	numIndices.push_back(indices1.size());
	...
	firstIndices.push_back(indices1.size());
	numIndices.push_back(indices2.size());

	std::vector<UINT> indices;
	indices.reserve(indices1.size() + indices2.size());
